...
const { width: imgContainerElementWidth } = useElementSize(imgContainerElement)
const {
...
const isNotUpdate = computed(
  () => isChapterQuery.value && !isEditChapterMode.value,
)
...
function isToolBarOverRightImageSide(
  ...
  return (
    elWidth + rect?.WIDTH / 2 + rect?.COMBINED_X - maxNaturalImageWidth.value >
    scaledOffSetX
  )
}
...
function noteOverImageWidthOffset(
  ...
  return elWidth +
    rect?.COMBINED_X +
    scaledOffSetX -
    0.85 * maxNaturalImageWidth.value >
    0
    ? elWidth / 2
    : ''
}
...
function handleOverImageHeight(
  ...
  const noteHeight = 46
  const scaledNoteHeight =
    (noteHeight / Number.parseFloat(transformScaleCssVar.value)) *
    props.zoomScaleRatio
  const offSetY =
    rect?.HEIGHT +
    rect?.COMBINED_Y +
    scaledNoteHeight -
    imgTransformContainerElementHeight.value
  return offSetY > 0 ? `${-(rect?.HEIGHT + scaledNoteHeight)}px` : '0'
}

const allRect = computed(() =>
  rectangleWithColorAndText.value?.map(
    (rect: ApiResponseModule.PostOcrGoogleVisionData) =>
      updateTextBoxMissingFields(rect),
  ),
)
const userCreateRect = computed(() =>
  allRect.value?.filter(
    (rect: ApiResponseModule.PostOcrGoogleVisionData) =>
      rect.TEXT_BOX_TYPE === TEXT_BOX_TYPES.MANUALLY,
  ),
)
const psdCreateRect = computed(() =>
  allRect.value?.filter(
    (rect: ApiResponseModule.PostOcrGoogleVisionData) =>
      rect.TEXT_BOX_TYPE === TEXT_BOX_TYPES.UPLOAD_PSD,
  ),
)
const excelCreateRect = computed(() =>
  allRect.value?.filter(
    (rect: ApiResponseModule.PostOcrGoogleVisionData) =>
      rect.TEXT_BOX_TYPE === TEXT_BOX_TYPES.IMPORT_EXCEL,
  ),
)
const visionCreateRect = computed(() =>
  allRect.value?.filter(
    (rect: ApiResponseModule.PostOcrGoogleVisionData) =>
      rect.TEXT_BOX_TYPE === TEXT_BOX_TYPES.OCR ||
      rect.TEXT_BOX_TYPE === TEXT_BOX_TYPES.BRING,
  ),
)
const userCreateNote = computed(() =>
  allRect.value?.filter(
    (rect: ApiResponseModule.PostOcrGoogleVisionData) =>
      rect.TEXT_BOX_TYPE === TEXT_BOX_TYPES.NOTE,
  ),
)

function updateTextBoxMissingFields(
  rect: ApiResponseModule.PostOcrGoogleVisionData,
) {
  if (!rect.ORIGINAL_TEXT_BOX_ID) rect.ORIGINAL_TEXT_BOX_ID = rect.ID
...
function reFocusEl() {
  if (props.isTranslatedImages) refInputTranslate.value?.focus()
  else refInputOriginal.value?.focus()
}
function handlerFocusAllText(
  rectangle: ApiResponseModule.PostOcrGoogleVisionData,
) {
  if (rectangle.TEXT_BOX_TYPE === TEXT_BOX_TYPES.NOTE) return
  if (!isEditRectangleContent.value) {
    timer.value = setTimeout(() => {
      const el = document.getElementById(
        `rectangle-content-${rectangle.ORIGINAL_TEXT_BOX_ID}-${props.isTranslatedImages ? 'translated' : 'original'}`,
      ) as HTMLElement
...
function handleRectDblClick(rect: ApiResponseModule.PostOcrGoogleVisionData) {
  onRemoveListener()
  if (isNotUpdate.value) return
  handlerFocusAllText(rect)
  isEditRectangleContent.value = true
}
function handleMovingArea() {
  const area = props.isTranslatedImages
    ? TRANSLATOR_LOADING_TYPES.TRANSLATE
    : TRANSLATOR_LOADING_TYPES.ORIGINAL
  emits('update:editorArea', area)
}
function handleRecClick(rect: ApiResponseModule.PostOcrGoogleVisionData) {
  const isUserCreateManualTextBox = [
    TEXT_BOX_TYPES.MANUALLY,
    TEXT_BOX_TYPES.NOTE,
  ].includes(rect.TEXT_BOX_TYPE)
  if (
    isNotUpdate.value ||
    (isUserCreateManualTextBox &&
      selectedOriginalId.value === rect.ORIGINAL_TEXT_BOX_ID) ||
    (!isUserCreateManualTextBox && selectedId.value === rect.ID)
  )
    return
  emits('handleActionIntoEditor', {
    actionName: ACTION_INTO_EDITOR.RESET_SELECTED_TEXT_BOX,
  })
  setTimeout(() => {
...
function updateStyleFromToolbar(
  rect: ApiResponseModule.PostOcrGoogleVisionData,
) {
  isUpdateFromToolbar.value = true
  const updatedStyleObject = _.pick(rect, [
    'FONT_SIZE',
    'TEXT_ALIGN',
    'BACKGROUND_COLOR',
  ])
...
function handleCachingRectStyle() {
  if (
    isUpdateFromToolbar.value ||
    !userCreateRect.value?.length ||
    props.drawNewNote
  )
    return
  const arr = userCreateRect.value?.sort((a, b) =>
    Date.parse(a.UPDATED_AT) >= Date.parse(b.UPDATED_AT) ? -1 : 1,
  )
  const firstItem = arr[0]
  if (firstItem) cachingRectStyle.value = firstItem
}
...
function getContentInRectangleEditing() {
  const el = document.getElementById(
    `rectangle-content-${selectedOriginalId.value}-${props.isTranslatedImages ? 'translated' : 'original'}`,
  ) as HTMLElement
...
async function updateSelectedRectangleContent() {
  ...
  const el = document.getElementById(
    `rectangle-content-${idElement}`,
  ) as HTMLElement
  if (el) {
    const found = props.rectangles?.find(
      (e) => e.ORIGINAL_TEXT_BOX_ID === selectedOriginalId.value,
    )
    if (found) {
      const content = replaceBreakLineHtml(el.innerHTML)
      if (found.TEXT_BOX_TYPE === TEXT_BOX_TYPES.NOTE) found.NOTE_TEXT = content
...
function drawingRectangle(
  ...
  if (!newRect) return
  if (props.drawNewNote) showNotes.value = true
  ...
  if (props.isTranslatedImages) {
    ...
    const rectangleStyle =
      newRect.TEXT_BOX_TYPE !== TEXT_BOX_TYPES.NOTE
    ...
    setTimeout(() => {
      const el = document.getElementById(
        `rectangle-content-${newRectangle.ORIGINAL_TEXT_BOX_ID}-translated`,
      )
      if (el) {
        isEditRectangleContent.value = true
...
function getTextBoxZIndex(
  ...
  switch (rectangle.TEXT_BOX_TYPE) {
    ...
    case TEXT_BOX_TYPES.IMPORT_EXCEL:
      zIndex =
        visionCreateRect.value.length + psdCreateRect.value.length + index
      break
    case TEXT_BOX_TYPES.MANUALLY:
      zIndex =
        visionCreateRect.value.length +
        psdCreateRect.value.length +
        excelCreateRect.value.length +
        index
      break
...
function checkEnabledArea(area: string | null) {
  return (
    (area === TRANSLATOR_LOADING_TYPES.TRANSLATE && props.isTranslatedImages) ||
...
function handleKeyDown(e: any) {
  ...
  const isAllowedKeyPressed =
    allowedCtrKey.includes(e.key) && (isMac ? e.metaKey : e.ctrlKey)
...
<template>
  ...
  >
    <input
      ...
      class="tw-fixed tw-top-1/2 tw-w-0"
      type="text"
    />
          ...
          <template v-if="maxNaturalImageWidth">
            ...
            >
              ...
              <div
                ...
                class="tw-absolute"
                :style="{
                  transform: `scale(${zoomScaleRatio})`,
                  zIndex:
              ...
              >
                <translate-image-note
                  ...
                  @on-add-new-note="
                    drawingRectangle({
                      ...note,
                      COMBINED_Y: note.COMBINED_Y + note.HEIGHT + 4,
                    })
                  "
                  ...
                  :style="{
                    ...
                    zIndex:
                      selectedOriginalId === rectangle.ORIGINAL_TEXT_BOX_ID
                        ? '999'
                        : getTextBoxZIndex(rectangle, index),
                  }"
                >
                  <translate-editor-toolbar
                    ...
                    :style="{
                      ...
                      transform: `${
                        isToolBarOverRightImageSide(
                          rectangle,
                          imgEditToolbarWidth,
                        )
                          ? 'translateX(20%)'
                          : 'translateX(-50%)'
                      }  scale(calc(1/var(--transform-scale) * var(--zoom-scale-ratio))`,
                    }"
                    ...
                    @update:model-value="
                      (newValue: ApiResponseModule.PostOcrGoogleVisionData) =>
                        (rectangle = newValue)
                    "
                  ...
                  >
                    ...
                    >
                      <img
                        v-if="
                          rectangle.PREFIX_INDEX ===
                          TEXT_BOX_PREFIX_INDEX.IMPORT_EXCEL
                        "
                      ...
                      <img
                        v-else-if="
                          rectangle.PREFIX_INDEX ===
                          TEXT_BOX_PREFIX_INDEX.UPLOAD_PSD
                        "
                    ...
                    <div
                      ...
                      class="rectangle-container tw-z-50 tw-outline-0 tw-whitespace-pre-line tw-select-none tw-w-full tw-h-full tw-flex tw-justify-center tw-flex-col tw-break-words img-editor-tbx"
                    ...
                    >
                      <div
                        ...
                        :contenteditable="
                          selectedOriginalId ===
                            rectangle.ORIGINAL_TEXT_BOX_ID &&
                          isEditRectangleContent
                        "
                        class="rectangle-content"
                        @keydown.delete="
                          deleteRectOnKeyDown(selectedOriginalId, {
                            ...rectangle,
                            TEXT: rectangle.TEXT,
                          })
                        "
                  ...
                  <div v-if="isTranslatedImages">
                    <translate-text-box-note
                      ...
                      :is-over-max-content="isOverMaxContent"
                      :offset-x="
                        noteOverImageWidthOffset(rectangle, imgNoteWidth)
                      "
                      :text-box-id="rectangle.ORIGINAL_TEXT_BOX_ID"
                      :is-edit="isEditChapterMode"
                      :style="{
                        transform: `translateY(${handleOverImageHeight(rectangle)}) scale(calc(1/var(--transform-scale) * var(--zoom-scale-ratio))`,
                      }"
                      @paste="checkPasteAsPlainText($event)"
                      @keydown="handleKeyDown($event)"
                    />
...
.resize-handle {
  ...
  border-radius: 50%;
  border: calc(2px / var(--transform-scale) * var(--zoom-scale-ratio)) solid
    #f38f84;
  --negative-center-offset: calc(
    -6px / var(--transform-scale) * var(--zoom-scale-ratio)
  );
...
.resize-handle-rotate {
  ...
  transform: translateX(-50%)
    translateY(calc(-50px / var(--transform-scale) * var(--zoom-scale-ratio)));
  z-index: 1;
  &:before {
...
.resize-handle-ne {
  ...
  transform: translateX(var(--center-offset))
    translateY(var(--negative-center-offset));
}
...
.resize-handle-sw {
  ...
  cursor: sw-resize;
  transform: translateX(var(--negative-center-offset))
    translateY(var(--center-offset));
}
